...
for (i in 1:length(noEntrezMappinghg38)) {
...
#genesFoundTot2<-rbind(genesFoundTot,genesFound2)
...
missedGenes2hg38<-c('LINC01410','LINC01909','LINC01480','ZFPM2-AS1',
	'SERPINB9P1','LRP1-AS','NRIR','TMIGD3','UBR5-AS1',
	'PYCARD-AS1','KLRA1P','KANTR','LOC102724323','LOC100505549',
	'LOC102724297','LOC100130357')

missedGenes2<-c('RP11-262H14.1','RP11-41O4.1','AC006129.4','RP11-152P17.2',
	'RP11-420G6.4','RP11-545N8.3','AC017076.5','ADORA3','KB-431C1.4',
	'C16orf98','KLRA1P','LINC01155','RP11-67C2.2','RP11-35G9.3',
	'RP11-24F11.2','RP1-257A7.4')

ensembl_id2<-c('ENSG00000238113','ENSG00000266258','ENSG00000270164','ENSG00000251003',
	'ENSG00000230438','ENSG00000259125','ENSG00000225964','ENSG00000121933','ENSG00000246263',
	'ENSG00000261359','ENSG00000256667','ENSG00000232593','ENSG00000231964','ENSG00000267040',
	'ENSG00000223552','ENSG00000215022')




for (i in 1:length(ensembl_id2)) {
	print(missedGenes2hg38[i])
	genesFound2<-rbind(genesFound2,c(missedGenes2hg38[i],ensembl_id2[i]))
}

genesFoundTot<-rbind(genesFoundTot,genesFound2)
save(genesFoundTot,file='genesFoundTot.Rdata')
