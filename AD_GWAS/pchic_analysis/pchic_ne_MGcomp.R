...
if ((exists("cli.args") == FALSE) | (length(cli.args) == 0)) {
  ...
  cli.args <- c()
  cli.args[1] <- "/shen/shenlabstore3/neng/20200309_MGEN"
...
# ~/figures/saved_MGcomp     - contains saved_MGcomp intermediate files used in the analysis
...
interactions.list <- 'interactions_fastp_250_5_MGcomp.txt'
atac.seq.peak.list <- 'atac_seq_peaks_optimal_MGcomp.txt'

# Read in interactions.
interactions.raw <- read.interactions(interactions.list, paste0(home.dir, "/pipeline"))
save(interactions.raw, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.raw.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.raw.Rdata"))
...
save(atac.seq.peaks, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/atac.seq.peaks.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/atac.seq.peaks.Rdata"))

# Read in features.
#features <- read.features(features.list, paste0(home.dir, "/pipeline"))
#save(features, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/features.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/features.Rdata"))
...
#save(transcript.lengths.individual, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/transcript.lengths.individual.Rdata"))
#save(transcript.counts.individual, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/transcript.counts.individual.Rdata"))
#save(transcript.tpm.individual, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/transcript.tpm.individual.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/transcript.lengths.individual.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/transcript.counts.individual.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/transcript.tpm.individual.Rdata"))
...
# Save results.
save(interactions.raw.processed, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.raw.processed.Rdata"))

print('Load Raw Processed interactions')
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.raw.processed.Rdata"))
...
if (!is.null(score.cutoff)) {
  ...
  for (cell.type in cell.types) {
    ...
    insig.binary<-!(interactions.raw.processed[[cell.type]]$interaction_ID %in% interactions.sig[[cell.type]]$interaction_ID)
    #interactions.low[[cell.type]] <- interactions.raw.processed[[cell.type]][which(insig.binary),]
    num.raw <- length(interactions.raw.processed[[cell.type]][, 1])
    num.sig <- length(interactions.sig[[cell.type]][, 1])
    #num.low <- length(interactions.low[[cell.type]][, 1])
...
# Save results.
save(interactions.sig, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.sig.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.sig.Rdata"))
...
for (cell.type in cell.types) {
  ...
  #interactions.low.res[[cell.type]] <- expandInteractions(interactions.low[[cell.type]], interaction.resolution, neighboring.fragments)
}


# Save results.
save(interactions.sig.res, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.sig.res.Rdata"))
#save(interactions.low.res, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.res.Rdata"))
save(interactions.all.res, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.all.res.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.sig.res.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.res.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.all.res.Rdata"))
...
# Save results.
save(atac.seq.peaks.res, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/atac.seq.peaks.res.Rdata"))
save(atac.seq.peaks.ann, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/atac.seq.peaks.ann.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/atac.seq.peaks.res.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/atac.seq.peaks.ann.Rdata"))
...
#  interactions.sig.ann[[cell.type]] <- annotateInteractions(interactions.sig.res[[cell.type]], atac.seq.peaks.res[[cell.type]], atac.seq.peaks.ann[[cell.type]], NULL, promoters.strict, promoters.other)
#  interactions.low.ann[[cell.type]] <- annotateInteractions(interactions.low.res[[cell.type]], atac.seq.peaks.res[[cell.type]], atac.seq.peaks.ann[[cell.type]], NULL, promoters.strict, promoters.other)
  
#}

# Save results.
#save(interactions.sig.ann, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.sig.ann.Rdata"))
#save(interactions.low.ann, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.ann.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.sig.ann.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.ann.Rdata"))
...
# Save results.
#save(interactions.low.res.sampled, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.res.sampled.Rdata"))
#save(interactions.low.ann.sampled, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.ann.sampled.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.res.sampled.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/interactions.low.ann.sampled.Rdata"))
...
# Save results.
save(hub.results.sig, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/hub.results.sig.Rdata"))
#save(hub.results.low, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/hub.results.low.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/hub.results.sig.Rdata"))
#load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/hub.results.low.Rdata"))
...
save(specificity.heatmap, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.heatmap.Rdata"))
print(paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.heatmap.Rdata"))
print('loading specificity heatmap')
load(paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.heatmap.Rdata"))
cell.types <- colnames(specificity.heatmap)
print(cell.types)
for (cell.type in cell.types) {
  specificity.heatmap.celltype <- specificity.heatmap[, cell.type]
  save(specificity.heatmap.celltype, file = paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.heatmap.", cell.type,'.Rdata', sep=""))
}

load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.heatmap.", cell.types[1], ".Rdata"))
specificity.matrix <- specificity.heatmap
for (cell.type in cell.types) {
  
  load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.heatmap.", cell.type, ".Rdata"))
...
# Save results.
save(specificity.matrix, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.matrix.Rdata"))
save(specificity.table, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.table.Rdata"))
save(specificity.entries, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.entries.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.matrix.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.table.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/specificity.entries.Rdata"))
...
shared.terms <- getSpecificInteractingGenes(interactions.sig.res, interactions.sig.ann, atac.seq.peaks.ann, specificity.entries, c("Microglia,ExcitatoryNeurons"), promoters.strict, rpkm.data.individual, 0.5, mode="both_atac")
...
# Save results.
save(merged.data.all, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/merged.data.all.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/merged.data.all.Rdata"))
...
# Save results.
save(vista.results, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/vista.results.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/vista.results.Rdata"))
...
# Save results.
save(all.snp.results, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/all.snp.results.Rdata"))
save(tag.snp.results, file=paste0(output.dir, "/", run.id, "/saved_MGcomp/tag.snp.results.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/all.snp.results.Rdata"))
load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/tag.snp.results.Rdata"))
...
for (cell.type in c("cortical", "motor")) {
  ...
  load(file=paste0(output.dir, "/", run.id, "/saved_MGcomp/allelic.interactions.sig.pval.", cell.type, ".Rdata"))
