...
date
hostname
HOME=/shen/shenlabstore3/neng/05242019_cutnrun_test/analysis
INDEX=$HOME/index.txt
SAMPLE=$(awk 'FNR == i {print}' i=${SGE_TASK_ID} $INDEX)
GENOME=hg38
#GENOME=hg19
#REFERENCE=/netapp/home/neng/reference_genomes/
LANE=/shen/shenlabstore3/neng/05242019_cutnrun_test
conda activate cutnrun

echo $LANE/${SAMPLE}*_R1*.f*.gz

fastp -i $LANE/*${SAMPLE}*_R1*.f*.gz -I $LANE/*${SAMPLE}*_R2*.f*.gz -o $HOME/${SAMPLE}.fastp.50bp.R1.fq.gz -O $HOME/${SAMPLE}.fastp.50bp.R2.fq.gz -p -h $HOME/"${SAMPLE}.fastp.html" -j $HOME/"${SAMPLE}.fastp.json"
...
bowtie2  --local --very-sensitive-local --no-mixed --no-discordant --phred33 -I 10 -X 700 -x /netapp/home/neng/test_genomes/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna.bowtie_index -1 ${input1} -2 ${input2} | samtools view -bS - > ${SAMPLE}.bam
