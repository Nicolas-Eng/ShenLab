...
## paths
rawdir=/shen/shenlabstore3/neng/20201116_SHAREseq/201112_A00351_0423_BHW3JJDRXX # e.g. ~/xxx/201021_SL-NVQ_0277_AHTHLLDRXX/ or ./test_fastq_nova/
dir=/shen/shenlabstore3/neng/20201116_SHAREseq/20201130_output_corrected
ymal=/shen/shenlabstore3/neng/20201116_SHAREseq/config_example2.yaml

## sample configuration
Project=(atac.lib rna.lib) # use differnt name for each sample
Type=(ATAC RNA)  # ATAC or RNA 
Genomes=(hg38 hg38) # both mm10 hg19
RawReadsPerBarcode=(100 100) # reads cutoff for the unfiltered bam, use 100 to remove barcode with few reads for “full" run and speed up processing ; use 10 for QC runs
ReadsPerBarcode=(100 100) # reads cutoff for the filtered bam 100 for full run of ATAC and RNA, 1 for QC run 
...
Start=Bcl # Bcl or Fastq_Merge (when fastq were generated per run) or Fastq_SplitLane (when fastq were generated per sequencing lane)
Runtype=full # QC or full;  QC only analyze 12M reads to get a quick sense of data
Sequencer=Nextseq # Novaseq or Nextseq;  miseq or nova-seq should use "Novaseq"
...
myPATH='/shen/shenlabstore3/neng/dependencies/SHARE-seq-alignment'
pythohPATH='/shen/shenlabstore3/neng/dependencies/anaconda3/envs/SHAREseq/bin/python'
picardPATH='/wynton/home/cbi/shared/software/CBI/picard-2.23.1/picard.jar'
...
export PATH="/shen/shenlabstore3/neng/dependencies/anaconda3/envs/SHAREseq/bin:$PATH"
...
if [ "$Start" = Bcl ]; then
	...
	mkdir $rawdir/fastqs/
	bcl2fastq -p $cores -R $rawdir --tiles s_[2] --no-lane-splitting --mask-short-adapter-reads 0 -o $rawdir/fastqs/ --create-fastq-for-index-reads  2>>$dir/Run.log
    fi
    cd $rawdir/fastqs/
	...
	for Species in ${Genome1[@]}; do
		...
		if [ ${Type[$index]} == "ATAC" ]; then
			...
			if [ ${Type[$index]} == "ATAC" ]; then
			    ...
			    parallel --will-cite --jobs 2 --delay 0.5 'if [ -f {.}.rmdup ]; then echo \"Found {.}.rmdup\"; else picard MarkDuplicates INPUT={} OUTPUT={.}.rmdup METRICS_FILE={.}.dups.log REMOVE_DUPLICATES=true VALIDATION_STRINGENCY=SILENT BARCODE_TAG=RG TMP_DIR='$dir'/temp/ 2>>'$dir'/Run.log && echo \"Marked duplicates for {}\"; fi' ::: SubBarcodes.$Name.$Species.*.bam
		...
		if [ ${Type[$index]} == "RNA" ]; then
			...
			if [ -f $dir/fastqs/$Name.$Species2.wdup.all.bam.bai ]; then
				...
				samtools view -H $dir/fastqs/$Name.$Species2.grouped.bam | cat - $dir/fastqs/$Name.$Species2.temp2.sam | \
